...
{
    void paint_radio_button(graphics& gfx, const region& rgn, const radio_button& rd_btn)
    {
        ...
        const auto bg_color = rd_btn.is_pressed() ? color{224, 224, 224} : color{255, 255, 255};
        gfx.set_draw_color(bg_color);

        const auto white = colors::make_white();
        const bool is_checked = rd_btn.is_checked();

        gfx.set_draw_color(is_checked ? color{0, 0, 128} : white); // Black color
        gfx.draw_fill_circle({x + 2, y + 2, size - 4, size - 4});

        gfx.set_draw_color(is_checked ? color{0, 0, 192} : white); // Black color
        gfx.draw_fill_circle({x + 3, y + 3, size - 6, size - 6});

        gfx.set_draw_color(is_checked ? color{128, 255, 128} : white); // Black color
