...
namespace clsn::ui::layouts
{
    ...
    void dual_layout::layout_vertical(const point& position,
                                      const dimension& size)
    {
        ...
        if (count == 2)
        {
            ...
            cc1.m_region = {{position.get_x(), position.get_y() + c0h},
                            {size.get_width(), c1h}};
        }
    }

    void dual_layout::layout_horizontal(const point& position,
                                        const dimension& size)
    {
        ...
        if (count == 2)
        {
            ...
            cc1.m_region = {{position.get_x() + c0w, position.get_y()},
                            {c1w, size.get_height()}};
        }
    }
    ...
    void dual_layout::layout(const region& rgn)
    {
        ...
        switch (m_orientation)
        {
            ...
            default: layout_vertical(position, size);
        }
    };
}