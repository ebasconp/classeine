    ...
    void HBoxContainer::initEvents() {
        ...
        addSizeChangedListener(
                [this](auto &e) {
                    ...
                    const int controlWidth =
                            getSize().getWidth() / count;
                    iterate([&controlWidth, this](auto &control) {
                        control.setSize(clsn::draw::Dimension{controlWidth, getSize().getHeight()});
                    });
                });

        addInvalidatedChangedListener([this](auto &e) {
            const bool isInvalidated = e.getNewValue();
            if (!isInvalidated)
                return;

            iterate([&isInvalidated](auto &control) {
